...
import { useProvideParentSizeAndViewportSize, ParentSizeState, constraintsEnabled, } from "../../render/types/NewConstraints.js";
...
export const Stack = React.memo(React.forwardRef(function Stack(stackProps, forwardedRef) {
    ...
    const gapChildren = wrapInGapElement(fractionChildren, gap, flexDirection, justifyContent, useFlexboxGap, wrap);
    // Stack children should ignore the viewport size
    const content = useProvideParentSizeAndViewportSize(gapChildren, parentSize ?? ParentSizeState.Disabled, null);
    const attributes = {
...
function wrapInGapElement(children, gap, direction, justifyContent, useFlexboxGap, wrap) {
    if (!isGapEnabled(gap, justifyContent, wrap)) {
        return children;
    }

...
import { constraintsEnabled, ParentSizeState, useConstraints, useProvideParentSizeAndViewportSize, } from "../../types/NewConstraints.js";
...
export function unwrapFrameProps(frameProps) {
    const { left, top, bottom, right, width, height, center, _constraints, size, widthType, heightType, positionFixed, } = frameProps;
...
function useStyleAndRect(props) {
    ...
    const shouldDisablePointerEvents = !hasEvents(props) && !props.__fromCanvasComponent && !hasScrollableContent(props) && !props.style?.pointerEvents;
...
const VisibleFrame = forwardRef(function VisibleFrame(props, forwardedRef) {
    ...
    const inCodeComponent = Boolean(useContext(ComponentContainerContext));
    const { parentSize, viewportSize } = resolveParentAndViewportSize(propsWithOverrides, unwrappedProps, rect, inCodeComponent);
    const wrappedContent = useProvideParentSizeAndViewportSize(React.createElement(React.Fragment, null,
        ...
        children,
        React.createElement(Border, { ..._border, border: border, layoutId: layoutId })), parentSize, viewportSize);
    ...
    return (React.createElement(MotionComponent, { ...dataProps, ...motionProps, layoutId: layoutId, style: currentStyle, ref: ref, transformValues: transformValues }, wrappedContent));
});
/** @internal */
...
function resolveParentAndViewportSize(props, unwrappedProps, rect, inCodeComponent) {
    if (inCodeComponent) {
        const parentSize = rect ? { width: rect.width, height: rect.height } : ParentSizeState.Disabled;
        return { parentSize, viewportSize: null };
    }
    const { _usesDOMRect, viewportHeight } = props;
    const { widthType = DimensionType.FixedNumber, heightType = DimensionType.FixedNumber, width, height, } = unwrappedProps;
    const hasViewportHeight = typeof viewportHeight === "number";
    ...
    if (rect && !_usesDOMRect) {
        return { parentSize: rect, viewportSize: hasViewportHeight ? { ...rect, height: viewportHeight } : null };
    }
    ...
    if (widthType === DimensionType.FixedNumber &&
        ...
        return {
            ...
            viewportSize: hasViewportHeight ? { width, height: viewportHeight } : null,
        };
    }
    ...
    if (_usesDOMRect) {
        return { parentSize: ParentSizeState.DisabledForCurrentLevel, viewportSize: null };
    }
    // If all else fails, return unknown
    return { parentSize: ParentSizeState.Unknown, viewportSize: null };
}

...
(function (ConstraintValues) {
    ...
    ConstraintValues.toMinSize = (values, parentSize, autoSize = null) => {
        ...
        return sizeAfterApplyingDefaultsAndAspectRatio(width, height, values, parentSize);
    };
    ...
    freeSpace = null, 
    /**
     * viewportSize is only required when called by useStyleAndRect in
     * FrameWithMotion, because only on the canvas do we convert percentage
     * height to a static value. Height is the only thing that should be
     * affected by viewportSize, the other constraint values should still be
     * parent-relative because on the canvas "fixed" elements are actually
     * position absolute.
     */
    viewportSize = null) => {
        let x = values.left || 0;
        ...
        if (parentHeight && isFiniteNumber(vOpposingPinsOffset)) {
            ...
            switch (values.heightType) {
                ...
                case DimensionType.Percentage:
                    if (viewportSize) {
                        height = viewportSize.height * values.height;
                    }

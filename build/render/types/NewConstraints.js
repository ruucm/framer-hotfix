...
/** @internal */
export function calculateRect(props, parentSize, pixelAlign = true, viewportSize = null) {
    if (!constraintsEnabled(props) ||
        parentSize === ParentSizeState.Disabled ||
        parentSize === ParentSizeState.DisabledForCurrentLevel) {
...
export const ProvideParentAndViewportSize = props => {
    ...
    const { parentSize, viewportSize, children } = props;
    const value = React.useMemo(() => ({ parentSize, viewportSize }), 
...
export function useProvideParentSizeAndViewportSize(node, parentSize, viewportSize) {
    return (React.createElement(ProvideParentAndViewportSize, { parentSize: parentSize, viewportSize: viewportSize }, node));
}
